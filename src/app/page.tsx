...
async function fetchStatus(): Promise<ServicesData> {
  const url = 'http://localhost:9090/api/v1/query?query=Shardeum';
  ...
  const services = jsonResponse.data.result.map((item: any) => ({
...
export default async function Home() {
  ...
  return (
    ...
    <main className="flex flex-col items-center p-24 bg-white">
      <h1 className="text-black font-regular my-2 text-xl text-left justify-start w-full max-w-[930px]">Current Systems Status </h1>
      <div className="flex w-full max-w-[930px] h-full gap-2 gap-y-5 p-2 items-center border flex-wrap">
        {services.map((service: Service, index: any) => (
          <div key={index} className="w-[450px] p-6 bg-white border border-gray-200 rounded-lg shadow flex justify-between">
            <div className="flex flex-col">
              ...
              <p className="font-normal text-gray-700 ">{service.status.value === 1 ? 'Operational' : 'Downtime'}</p>
            </div>
